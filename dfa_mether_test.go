...
func TestMatchChinese(t *testing.T) {
	...
	replaceChar := '*'
	filteredWords, replacedText := NewDFAMather().Builder(filterChineseWords).Match(chineseText, replaceChar)
...
func TestMatchEnglish(t *testing.T) {
	...
	replaceChar := '*'
	filteredWords, replacedText := NewDFAMather().Builder(filterEnglishWords).Match(englishText, replaceChar)
	assert.Equal(t, len(filteredWords), 7)
...
func TestMatchMixLanguage(t *testing.T) {
	...
	replaceChar := '*'
	filteredWords, replacedText := NewDFAMather().Builder(filterWords).Match(text, replaceChar)
	assert.Equal(t, len(filteredWords), 7)
